...
// gunakan Light sebagai tema awal. Setelah admin memilih manual,
...
function storeCollection(name) {
  ...
  return KQServices.firestore.storeCollection(name, tokoId, new Set([name]));
}
...
function prosesLogin(e) {
  ...
  KQServices.auth.signIn(email, password)
    .then(() => {
...
function logout() {
  if (confirm("Apakah Anda yakin ingin keluar?")) KQServices.auth.signOut().catch(err => alert("Gagal keluar: " + err.message));
}
