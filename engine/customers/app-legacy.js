    ...
    function storeCollection(name) {
      return KQServices.firestore.storeCollection(name, ACTIVE_TOKO_ID, V13_STORE_COLLECTIONS);
    }
    ...
    window.onload = function() {
      ...
      const cachedStoreName = localStorage.getItem('cust_store_name_v13'); if (cachedStoreName) { const welcomeName = document.getElementById('welcomeStoreName'); if (welcomeName) welcomeName.innerText = cachedStoreName; document.getElementById('receipt-shop-name').innerText = cachedStoreName; if(document.getElementById('customer-home-store-name')) document.getElementById('customer-home-store-name').innerText = cachedStoreName; }
    ...
    function applyThemePelanggan(theme) { return window.KQTheme.applyCustomer(theme); }
    function gantiTemaPelanggan(theme) { return window.KQTheme.changeCustomer(theme); }
    ...
    function switchTabPelanggan(tabId) {
      ...
      if (window.syncCustomerShellNav) window.syncCustomerShellNav(tabId);
      const chatBadge = document.getElementById('kq-cust-chat-badge'); const legacyChatBadge = document.getElementById('badge-livechat-cust'); if (chatBadge && legacyChatBadge) { const visible = getComputedStyle(legacyChatBadge).display !== 'none'; chatBadge.textContent = legacyChatBadge.textContent || '0'; chatBadge.style.display = visible ? 'flex' : 'none'; chatBadge.setAttribute('aria-hidden', visible ? 'false' : 'true'); }
