...
// ==========================================================
// OPTIMASI KUOTA FIREBASE (GET SEKALI AMBIL & BERURUTAN)
// ==========================================================
...
function prosesSimpanTransaksi() {
  ...
  db.collection("transaksi").add(transaksi).then(() => {
    muatDataProduk();
  }).catch(err => console.error("Gagal simpan transaksi ke cloud: ", err));
}
...
// ==========================================================
// INISIALISASI UTAMA (BERURUTAN & AMAN DARI ERROR KOSONG)
// ==========================================================
async function inisialisasiAplikasi() {
  await muatDataProduk();
  await muatDataPelanggan();
}
...
// ==========================================================
// OPTIMASI KUOTA FIREBASE (DIUBAH DARI onSnapshot MENJADI get())
// ==========================================================
...
// Panggil fungsi pemuatan awal data produk dan pelanggan sekali ambil
muatDataProduk();
muatDataPelanggan();
...
function simpanBarangLangsung() {
  ...
  db.collection("produk").doc(code).set({
    ...
    closeProductModal();
    showNotif("Barang baru ditambahkan ke stok!");
    muatDataProduk(); // Refresh manual setelah simpan
  }).catch(err => alert("Gagal menyimpan: " + err.message));
}
...
function simpanEditBarang(code) {
  ...
  db.collection("produk").doc(code).update({
    ...
    closeProductModal();
    showNotif("Perubahan disimpan!");
    muatDataProduk(); // Refresh manual setelah edit
  }).catch(err => alert("Gagal menyimpan: " + err.message));
}
  ...
  if (cust) {
    ...
    db.collection("pelanggan").doc(custId).update({ catatan: cust.catatan })
      .then(() => { 
        closeBookkeepingModal(); 
        showNotif("Catatan disimpan!"); 
        muatDataPelanggan(); // Refresh manual setelah simpan catatan
      })
      .catch(err => alert("Gagal menyimpan: " + err.message));
  }
}
  ...
  if (confirm("Hapus catatan ini?")) {
    ...
    if (cust && cust.catatan) {
      ...
      db.collection("pelanggan").doc(custId).update({ catatan: cust.catatan })
        .then(() => { 
          showNotif("Catatan dihapus!"); 
          muatDataPelanggan(); // Refresh manual setelah hapus catatan
        })
        .catch(err => alert("Gagal menghapus: " + err.message));
    }
  }
...
async function prosesBelanjaStok() {
  ...
  restockListItems = [];
  simpanRestockKeCloud();
  muatDataProduk(); // Refresh data produk setelah restock
  alert("🎉 Belanja berhasil diproses! Stok, modal, dan harga jual di Halaman Stok telah diperbarui.");
  switchTab('data-barang', false);
}
...
function prosesSimpanTransaksi() {
  ...
  db.collection("transaksi").add(transaksi).then(() => {
    muatDataProduk(); // Perbarui data produk setelah transaksi
  }).catch(err => console.error("Gagal simpan transaksi ke cloud: ", err));
}
...
function hapusBarang(code) {
  if(confirm("Hapus barang ini?")) {
    db.collection("produk").doc(code).delete()
      .then(() => {
        showNotif("Barang dihapus!");
        muatDataProduk(); // Refresh manual setelah hapus barang
      })
      .catch(err => alert("Gagal menghapus: " + err.message));
  }
}
...
async function importStokExcel(event) {
  ...
  reader.onload = async function(e) {
    ...
    alert(`Berhasil mengimpor ${count} data!`);
    document.getElementById("import-csv-file").value = "";
    muatDataProduk(); // Refresh data produk setelah impor
  };
  reader.readAsText(file);
}
...
function importPelangganExcel(event) {
  ...
  reader.onload = async function(e) {
    ...
    alert(`Berhasil mengimpor ${count} pelanggan!`);
    document.getElementById("import-cust-file").value = "";
    muatDataPelanggan(); // Refresh data pelanggan setelah impor
  };
  reader.readAsText(file);
}
...
function simpanPelanggan() {
  ...
  db.collection("pelanggan").doc(custId).set(dataPelanggan, { merge: true })
    .then(() => { 
      closeCustomerModal(); 
      showNotif("Pelanggan tersimpan!"); 
      muatDataPelanggan(); // Refresh manual setelah simpan pelanggan
    })
    .catch(err => alert("Gagal: " + err.message));
}
...
function setujuiAkunPelanggan(docId) {
  db.collection("pelanggan").doc(docId).update({
    ...
    alert("Akun pelanggan berhasil disetujui! Pelanggan sekarang dapat masuk.");
    muatDataPelanggan(); // Refresh manual setelah setujui akun
  }).catch(err => {
    alert("Gagal menyetujui akun: " + err.message);
  });
}
...
function hapusPelanggan(id) {
  if (confirm("Hapus pelanggan ini?")) {
    db.collection("pelanggan").doc(id).delete()
      .then(() => { 
        showNotif("Pelanggan dihapus!"); 
        muatDataPelanggan(); // Refresh manual setelah hapus pelanggan
      })
      .catch(err => alert("Gagal: " + err.message));
  }
}
