...
// ==========================================================
// FITUR BACKUP & RESTORE SELURUH DATA (DATABASE FIREBASE)
// ==========================================================

async function eksporSeluruhData() {
  try {
    showNotif("Menyiapkan backup data...");
    
    // Siapkan wadah untuk seluruh koleksi Firestore
    const allData = { produk: {}, pelanggan: {}, transaksi: {}, pengaturan: {}, chats: {} };
    const collections = Object.keys(allData);

    // Looping untuk mengambil data dari setiap koleksi
    for (let col of collections) {
      const snapshot = await db.collection(col).get();
      snapshot.forEach(doc => {
        allData[col][doc.id] = doc.data();
      });
    }

    // Ubah objek data menjadi string JSON yang rapi
...
async function imporSeluruhData(event) {
  ...
  if (!confirm("⚠️ PERINGATAN: Impor data akan menimpa/menggabungkan data yang ada di server saat ini dengan data dari file. Lanjutkan?")) {
  ...
  reader.onload = async function(e) {
    try {
      ...
      const collections = ['produk', 'pelanggan', 'transaksi', 'pengaturan', 'chats'];
      ...
      for (let col of collections) {
        if (importedData[col]) {
          for (let docId in importedData[col]) {
            let docRef = db.collection(col).doc(docId);
            
            // Menggunakan { merge: true } agar data baru tertimpa namun parameter yang tidak ada di backup tidak terhapus
            batch.set(docRef, importedData[col][docId], { merge: true }); 
            count++;
            totalImported++;

            // Batas maksimal batch Firestore adalah 500, kita commit per 450 dokumen
            if (count >= 450) {
      ...
      // Commit sisa data yang belum masuk kelipatan 450
      if (count > 0) {
        commitPromises.push(batch.commit());
      }

      showNotif("Sedang mengunggah ke Cloud...");
      await Promise.all(commitPromises); // Tunggu semua proses penulisan batch selesai
      
      alert(`🎉 Berhasil merestore total ${totalImported} dokumen data ke sistem!`);
      event.target.value = ""; // Reset input
      
      // Muat ulang UI dengan data terbaru
      refreshData();
      
    } catch (err) {
      alert("❌ Gagal mengimpor data. Pastikan format file JSON valid. Error: " + err.message);
      event.target.value = "";
    }
  };
